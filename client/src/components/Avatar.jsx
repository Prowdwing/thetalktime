...
export default function Avatar({ user, size = 'md', className = '' }) {
    ...
    return (
        <div className={`relative rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center font-bold text-white shadow-sm border border-black/5 dark:border-white/10 ${sizeClasses[size] || sizeClasses.md} ${hasImage ? 'bg-gray-100' : getColor(user?.displayName)} ${className}`}>
            {hasImage ? (
                <img
